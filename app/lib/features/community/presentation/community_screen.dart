...
class _CommunityScreenState extends ConsumerState<CommunityScreen> {
  ...
  Future<void> _pickRegionAsGuest() async {
    final selected = await showRegionSelectSheet(context, prompt: '어느 지역 커뮤니티로 갈까요?');
            ...
            child: ListView(
              ...
              children: [
                ...
                authAsync.when(
                  data: (isLoggedIn) {
                    if (isLoggedIn) {
                      return _MemberSection(onChangeRegion: _pickRegionAsMember);
                    }
                    if (!_enteredAsGuest) {
                    ...
                    return _RegionBoardsSection(region: _guestRegion!, onChangeRegion: _pickRegionAsGuest);
                  },
                  loading: () => const Padding(
                    padding: EdgeInsets.symmetric(vertical: 40),
                    child: Center(child: CircularProgressIndicator(color: AppColors.accent)),
                  ),
                  error: (_, _) => _LoginOrGuestCard(onGuest: () => setState(() => _enteredAsGuest = true)),
                ),
              ],
            ),
          ),
      ...
      child: Column(
        children: [
          Container(
            ...
            child: const Icon(Icons.location_city_outlined, color: AppColors.accentDeep, size: 26),
          ),
          const SizedBox(height: 14),
          Text('어느 지역 커뮤니티로 갈까요?', style: AppTypography.headline, textAlign: TextAlign.center),
          const SizedBox(height: 6),
          Text('지역구를 고르면 그 동네 게시판으로 이동해요', style: AppTypography.subhead, textAlign: TextAlign.center),
          const SizedBox(height: 16),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return profileAsync.when(
      data: (profile) {
        ...
        return _RegionBoardsSection(region: region, onChangeRegion: onChangeRegion);
      },
      loading: () => const Padding(
        padding: EdgeInsets.symmetric(vertical: 40),
        child: Center(child: CircularProgressIndicator(color: AppColors.accent)),
...
class _RegionBoardsSection extends StatelessWidget {
  const _RegionBoardsSection({required this.region, required this.onChangeRegion});
...
String shortRegionLabel(String region) {
  ...
  for (final suffix in suffixes) {
    ...
    if (index > 0) {
      return (region.substring(0, index) + region.substring(index + suffix.length)).trim();
    }
  }
  return region;
}
...
class _BoardRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return InkWell(
      onTap: () => context.push('/community/${Uri.encodeComponent(region)}/${board.id}'),
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            SizedBox(
              ...
              child: Text(
                board.label,
                style: AppTypography.subhead.copyWith(color: AppColors.ink, fontWeight: FontWeight.w600),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
              ),
            ),
            ...
            if (latest != null)
              ...
              const Icon(Icons.chevron_right, size: 18, color: AppColors.inkTertiary),
          ],
        ),
      ),
    );
