...
/// 전부 같은 흐름을 쓰되, 추억 게시판만 사진이 사실상 필수다.
Future<void> showCommunityPostFlow(
  ...
  if (board.requiresPhoto) {
    ...
    requiredPhoto = await ImagePicker().pickImage(source: ImageSource.gallery, imageQuality: 85);
    if (requiredPhoto == null || !context.mounted) return;
  }
  ...
  final details = await showModalBottomSheet<_PostDetails>(
    ...
    builder: (context) => _PostDetailsSheet(board: board, requiredPhoto: requiredPhoto),
  );
  ...
  try {
    ...
    final photo = requiredPhoto ?? details.optionalPhoto;
    if (photo != null) {
...
class _PostDetails {
  const _PostDetails({this.title, this.caption, this.year, this.optionalPhoto});
  final String? title;
  final String? caption;
  final int? year;
  final XFile? optionalPhoto;
}

class _PostDetailsSheet extends StatefulWidget {
  const _PostDetailsSheet({required this.board, this.requiredPhoto});

  final CommunityBoard board;
  final XFile? requiredPhoto;
...
class _PostDetailsSheetState extends State<_PostDetailsSheet> {
  ...
  Future<void> _pickOptionalPhoto() async {
    final permission = await Permission.photos.request();
    if (!permission.isGranted) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
    ...
    if (picked != null && mounted) setState(() => _optionalPhoto = picked);
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if (showOptionalPhotoPicker) ...[
            ...
            OutlinedButton.icon(
              onPressed: _pickOptionalPhoto,
              icon: const Icon(Icons.add_a_photo_outlined, size: 18),
              label: Text(_optionalPhoto == null ? '사진 추가 (선택)' : '사진 1장 첨부됨'),
            ),
          ],
