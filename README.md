...
alembic upgrade head          # 로그인/프로필용 테이블 생성 (Postgres 필요)
...
`/api/auth/*`, `/api/profile*`는 실제 Postgres 연결이 필요합니다(로그인·저장한 골목 등
사용자 데이터를 저장하므로). 그 외 엔드포인트(`/api/location`, `/api/course`,
`/api/archive`)는 Postgres 없이도 목 데이터로 동작합니다.
...
홈 화면(추억)이 옛길 디자인 시스템(베이지 배경 + 흰 카드 + 브라운 포인트)으로 뜨면 정상입니다.
...
로그인(카카오)과 프로필(저장한 골목 등)을 쓰려면 `JWT_SECRET_KEY`(직접 `openssl rand -hex 32`로 생성)와 `KAKAO_REST_API_KEY`(카카오 개발자센터, "카카오 로그인" 활성화 후 발급)를 채워야 합니다. 실제 배포 절차는 [`infra/DEPLOY.md`](infra/DEPLOY.md)를 참고하세요.
